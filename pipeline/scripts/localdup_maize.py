...
from find_cns_maize import parse_blast, get_masked_fastas
...
def main(cns_file,qdups_path,sdups_path,pair_file,unmasked_fasta,fmt,qbed,sbed,qpad,spad,blast_path,mask='F',ncpu=8):
    pool = Pool(ncpu)
    bl2seq = "%s " % blast_path + \
...
if __name__ == "__main__":
    ...
    main(options.cns_file,options.qdups,options.sdups,options.pairs,unmasked_fasta,options.pair_fmt,qbed,sbed,options.qpad,options.spad,options.blast_path,options.mask,options.ncpu)
